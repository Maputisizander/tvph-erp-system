...
<span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${statusBadgeClasses(po.status)}`}>
                  {po.status === 'pending_approval' ? 'For Approval' : po.status === 'pending_finance' ? 'Finance Approval' : po.status === 'signed' ? 'Awaiting Signature' : po.status.replace('_', ' ').charAt(0).toUpperCase() + po.status.replace('_', ' ').slice(1)}
                </span>
                ...
                {po.requirements_waived && po.waiver_approved && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border bg-teal-50 text-teal-700 border-teal-200 dark:bg-teal-900/20 dark:text-teal-400 dark:border-teal-800/50">
