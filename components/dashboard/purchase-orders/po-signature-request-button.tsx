...
export function PoSignatureRequestButton({ poId, menu = false }: { poId: string; menu?: boolean }) {
  ...
  function handleClick() {
    ...
    startTransition(async () => {
      const result = await sendPOForSignature(poId);
      if (result?.error) {
        setFeedback({ ok: false, msg: result.error });
      } else {
        setFeedback({
          ok: true,
          msg: result.emailWarning ? `Signature request sent. ${result.emailWarning}` : "Signature request sent to vendor.",
        });
        router.refresh();
      }
    });
  ...
  return (
    <div className={menu ? "flex flex-col w-full" : "flex flex-col items-end gap-1"}>
      ...
      >
        ...
        {isPending ? "Sending…" : "Request Signature"}
      </button>
      {feedback && (
        <span
