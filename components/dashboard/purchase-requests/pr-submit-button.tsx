...
import { submitPRForApproval } from "@/app/dashboard/purchase-requests/actions";
...
export function PrSubmitButton({
  ...
  function handleSubmit() {
    ...
    startTransition(async () => {
      const result = await submitPRForApproval(prId, selected, selectedFinance);
      if (result?.error) {
    ...
    return (
      <div className="flex flex-col items-end gap-1">
        <button
          ...
          title={
            noApprovers
              ? "No other admin or superadmin is available to approve this PR."
        ...
        <span className="text-xs text-amber-600 dark:text-amber-400">
          {noApprovers
            ? "No eligible approver — another admin must exist to approve this PR."
