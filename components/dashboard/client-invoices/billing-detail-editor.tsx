      ...
      <>
        <div className="glass-card rounded-2xl p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
            ...
            <div><p className="text-xs text-slate-400">Date Issued</p><p className="font-medium text-slate-900 dark:text-white mt-0.5">{row.date_issued ? new Date(row.date_issued).toLocaleDateString("en-US", { timeZone: "Asia/Manila", month: "short", day: "numeric", year: "numeric" }) : '—'}</p></div>
            <div><p className="text-xs text-slate-400">Date Endorsed</p><p className="font-medium text-slate-900 dark:text-white mt-0.5">{row.date_endorsed ? new Date(row.date_endorsed).toLocaleDateString("en-US", { timeZone: "Asia/Manila", month: "short", day: "numeric", year: "numeric" }) : '—'}</p></div>
            <div><p className="text-xs text-slate-400">Due Date</p><p className="font-medium text-slate-900 dark:text-white mt-0.5">{row.due_date ? new Date(row.due_date).toLocaleDateString("en-US", { timeZone: "Asia/Manila", month: "short", day: "numeric", year: "numeric" }) : '—'}</p></div>
            <div><p className="text-xs text-slate-400">Est. Payment</p><p className="font-medium text-slate-900 dark:text-white mt-0.5">{row.est_payment_date ? new Date(row.est_payment_date).toLocaleDateString("en-US", { timeZone: "Asia/Manila", month: "short", day: "numeric", year: "numeric" }) : '—'}</p></div>
            <div><p className="text-xs text-slate-400">Collected At</p><p className="font-medium text-emerald-700 dark:text-emerald-400 mt-0.5">{row.collected_at ? new Date(row.collected_at).toLocaleDateString("en-US", { timeZone: "Asia/Manila", month: "short", day: "numeric", year: "numeric" }) : '—'}</p></div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
              {nodes.map((n, idx) => (
                <tr key={idx} className="group hover:bg-slate-50/50 dark:hover:bg-slate-800/10">
                  ...
                  <td className={td}><input type="text" value={n.node_id} title={n.node_id} onChange={e => updateNode(idx, "node_id", e.target.value)} onPaste={handlePaste} className={inpDark} placeholder="MN113" /></td>
